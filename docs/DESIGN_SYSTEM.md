...
## 18. Acréscimos deste projeto (Customer Experience)

Decisões pedidas pela equipe CX que estendem as regras acima. Os tokens ficam no fim de
`src/index.css`, marcados.

| Acréscimo | Onde | Regra |
|---|---|---|
| **Traço amarelo** `--accent` (`#f3bb13` / `#e8b317`) | `AccentRule` (48×2px): sob o título de cada página e sob a marca na sidebar | Detalhe, nunca sinal. Não vai em card, lista nem botão. |
| **Prioridade colorida** | `PrioridadeStatus` | Alta: ponto + palavra em vermelho (enfatizada). Média: ponto amarelo (`--accent`), palavra em tinta. Baixa: ponto cinza. O grau vem da posição do nível na lista (`grauDoNivel`), não do nome. É a segunda exceção ao "vermelho só em crítico". |
| **Caixa de contraste** `Card tone="contrast"` | os números do Painel | Uma por tela, no azul Anchieta escolhido pela equipe (`--contraste`: `#0e3a66` / `#0f355e`). O card recebe a classe `.dark` e os tokens de dentro viram os do tema escuro, com tinta clara sobre o azul. `--contraste` segue o `<html>`, para cada tema ter a sua variante. É mais uma exceção à Regra 4, que reserva o azul para três lugares. |
| **Caixa de contraste em Concluídos** | os quatro números do período | A mesma caixa do Painel. É a única da tela de Concluídos. |
| **Tendência invertida** `TrendIndicator invertido` | tempo médio até concluir | Para o número em que menos é melhor: a subida ganha o vermelho, a queda fica neutra. A seta continua dizendo a direção. |
| **Ponto de novidade** `NewDot` | sino de novidades no header | 8px em `--brand-2`, sem número, com anel na cor da superfície. Só avisa que vale abrir; contar seria mais um número no header. |
| **Status do TI** `StatusTiBadge` | quadro, lista, painel, Fila do TI | "Em desenvolvimento" é o estado saudável (só a palavra). "Na fila do TI" leva o ponto apagado. "Pronto para validar" é enfatizado em `info`: é a vez do CX agir. |
| **Quem está usando** | pé da sidebar, acima de Configurações | Saiu do header: é onde se procura "a minha conta", e o header fica com as ações. |
